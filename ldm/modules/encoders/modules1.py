...
class FrozenCLIPEmbedder(AbstractEncoder):
    ...
    def __init__(self, version="/nas/longleaf/home/debman/air-diffusion/UniControl/openai/clip-vit-large-patch14", device="cuda", max_length=77):  # clip-vit-base-patch32
        super().__init__()
...
class FrozenCLIPEncoder(AbstractEncoder):
    """Uses the CLIP transformer encoder for text (from huggingface)"""
    ...
    def forward(self, text):
        ...
        z = outputs.pooler_output # [batch, 768]
        return z
...
class FrozenCLIPImageEmbedder(AbstractEncoder):
    ...
    def preprocess(self, x):
        ...
        x = (x + 1.) / 2.
        # renormalize according to clip
        # print(x.shape, self.mean.shape, self.std.shape)
        x = kornia.enhance.normalize(x, self.mean, self.std)
        return x
    ...
    def forward(self, x):
        ...
        if isinstance(x, list):
            ...
            return torch.zeros(len(x), 768, device=device)
...
class FrozenCLIPImageMutliEmbedder(AbstractEncoder):
    ...
    def forward(self, x):
        ...
        if isinstance(x, list):
            # [""] denotes condition dropout for ucg
            device = self.model.visual.conv1.weight.device
            return torch.zeros(len(x), self.max_crops, 768, device=device)
            # x = torch.stack(x, dim=0)
        batch_tokens = []
        for im in x:
...
class TaskMixerEmbedder(AbstractEncoder):
    """
        Uses the CLIP image and text encoder.
        Not actually frozen... If you want that set cond_stage_trainable=False in cfg
        """
    def __init__(
            self,
    ...
    def forward(self, x):
        ...
        if isinstance(x, list):
            ...
            btrain = x['train']
            batch_tokens = []
            for im, txt, task, _train in zip(bim, btxt, btask, btrain):
                if txt == '':
                    txt_tokens = torch.zeros(0, 768, device=device)
                else:
                    txt_token_input = clip.tokenize(txt, truncate=True).to(device)
                    txt_tokens = self.model.encode_text(txt_token_input).float()
                if task == '':
                    task_tokens = torch.zeros(0, 768, device=device)
                else:
                    task_token_input = clip.tokenize(task, truncate=True).to(device)
                    task_tokens = self.model.encode_text(task_token_input).float()
                # print(im.shape)
                patches = self.preprocess(im.permute(2, 0, 1).unsqueeze(0))
                im_tokens = self.model.encode_image(patches).float()
                if _train:
                    for t in im_tokens:
                        if random.random() < 0.4:
                            t *= 0
                tokens = torch.cat([im_tokens, txt_tokens, task_tokens], dim=0)[-self.max_crops:]
                # tokens = im_tokens
...
class TaskMixerEmbedderInference(AbstractEncoder):
    """
        Uses the CLIP image and text encoder.
        Not actually frozen... If you want that set cond_stage_trainable=False in cfg
        """
    ...
    def forward(self, x):
        ...
        if isinstance(x, list):
            ...
            btask = x['task']
            batch_tokens = []
            for im, txt, task in zip(bim, btxt, btask):
                if txt == '':
                    txt_tokens = torch.zeros(0, 768, device=device)
                else:
                    txt_token_input = clip.tokenize(txt, truncate=True).to(device)
                ...
                #     if random.random() < 0.1:
                #         t *= 0
                tokens = torch.cat([im_tokens, txt_tokens, task_tokens], dim=0)[-5:]
                # tokens = im_tokens
